export class BabylonRenderer {
    ...
    setupEnvironment() {
        ...
        const sunLight = new BABYLON.DirectionalLight("sunLight", new BABYLON.Vector3(-0.35, -1, -0.25), this.scene);
        ...
        const ground = BABYLON.MeshBuilder.CreateGround("sceneGround", { width: 260, height: 260 }, this.scene);
    ...
    createSurfaceMaterial(name, color, roughness, metallic = 0, alpha = 1) {
    ...
    createGlassMaterial(name, color, alpha = 0.38) {
    ...
    setSunPosition(position, intensity) {
        if (!this.sunLight) {
            return;
        }
        ...
        try {
            ...
            result.forEach(mesh => {
                ...
                if (typeof model.properties.alpha === 'number' && mesh.material && 'alpha' in mesh.material) {
                    mesh.material.alpha = model.properties.alpha;
                }
    ...
    createMeshFromModel(model) {
        try {
            ...
            switch (type) {
                case 'wall':
                    return this.createWall(model.properties.width || 5, model.properties.height || 4, model.properties.depth || 0.3, model.properties.position, model.properties.name);
                case 'floor':
                    return this.createFloor(model.properties.width || 5, model.properties.height || 0.1, model.properties.depth || 5, model.properties.position, model.properties.name);
                case 'door':
                    return this.createDoor(model.properties.width || 1, model.properties.height || 2.2, model.properties.depth || 0.1, model.properties.position, model.properties.name);
                case 'window':
                    return this.createWindow(model.properties.width || 1, model.properties.height || 1, model.properties.depth || 0.1, model.properties.position, model.properties.name);
                case 'foundation':
                    return this.createFoundation(model.properties.width || 5, model.properties.height || 0.5, model.properties.depth || 5, model.properties.position, model.properties.name);
                case 'roof':
    ...
    createWall(width, height, depth, position, name) {
        try {
            ...
            const material = this.createSurfaceMaterial(`${name}_mat`, accentLike ? new BABYLON.Color3(0.78, 0.79, 0.82) : new BABYLON.Color3(0.72, 0.7, 0.67), accentLike ? 0.38 : 0.9, accentLike ? 0.32 : 0);
    ...
    createFloor(width, height, depth, position, name) {
        try {
    ...
    createDoor(width, height, depth, position, name) {
        try {
    ...
    createWindow(width, height, depth, position, name) {
        try {
    ...
    createFoundation(width, height, depth, position, name) {
        try {
                ...
                try {
                    if (mesh === roof) {
                        ...
                        if (this.scene.getPhysicsEngine()) {
                            mesh.physicsImpostor = new BABYLON.PhysicsImpostor(mesh, BABYLON.PhysicsImpostor.BoxImpostor, { mass: 1, restitution: 0.2, friction: 0.4 }, this.scene);
                        }
                    }
                        ...
                        if (this.scene.getPhysicsEngine()) {
                            mesh.physicsImpostor = new BABYLON.PhysicsImpostor(mesh, BABYLON.PhysicsImpostor.BoxImpostor, { mass: 1, restitution: 0.9, friction: 0.5 }, this.scene);
                        }
                    }
                }
                catch (e) {
